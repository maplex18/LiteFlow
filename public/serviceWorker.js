const CHATGPT_lite_WEB_CACHE = "chatgpt-lite-web-cache";
const CHATGPT_lite_WEB_FILE_CACHE = "chatgpt-lite-web-file";
...
self.addEventListener("install", function (event) {
  ...
  event.waitUntil(
    caches.open(CHATGPT_lite_WEB_CACHE).then(function (cache) {
      return cache.addAll([]);
    }),
  );
});
...
async function upload(request, url) {
  ...
  const cache = await caches.open(CHATGPT_lite_WEB_FILE_CACHE)
