...
export class HunyuanApi implements LLMApi {
  async path(): Promise<string> {
    ...
    if (baseUrl.length === 0) {
      try {
        ...
        console.error("[Tencent] Failed to get client config:", error);
        baseUrl = ApiPath.Tencent; // Default to API path if config fetch fails
      }
    }
  ...
  async chat(options: ChatOptions) {
    ...
    try {
      const chatPath = await this.path();
      const chatPayload = {
        method: "POST",
