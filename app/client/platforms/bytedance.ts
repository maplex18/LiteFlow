...
export class DoubaoApi implements LLMApi {
  async path(path: string): Promise<string> {
    ...
    if (baseUrl.length === 0) {
      try {
        ...
        console.error("[ByteDance] Failed to get client config:", error);
        baseUrl = ApiPath.ByteDance; // Default to API path if config fetch fails
      }
    }
  ...
  async chat(options: ChatOptions) {
    ...
    try {
      const chatPath = await this.path(ByteDance.ChatPath);
      const chatPayload = {
        method: "POST",
