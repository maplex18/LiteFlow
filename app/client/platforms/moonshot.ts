...
export class MoonshotApi implements LLMApi {
  ...
  async path(path: string): Promise<string> {
    ...
    if (baseUrl.length === 0) {
      try {
        ...
        console.error("[Moonshot] Failed to get client config:", error);
        baseUrl = ApiPath.Moonshot; // Default to API path if config fetch fails
      }
    }
  ...
  async chat(options: ChatOptions) {
    ...
    try {
      const chatPath = await this.path(Moonshot.ChatPath);
      const chatPayload = {
        method: "POST",
