...
export class XAIApi implements LLMApi {
  ...
  async path(path: string): Promise<string> {
    ...
    if (baseUrl.length === 0) {
      try {
        ...
        console.error("[XAI] Failed to get client config:", error);
        baseUrl = ApiPath.XAI; // Default to API path if config fetch fails
      }
    }
  ...
  async chat(options: ChatOptions) {
    ...
    try {
      const chatPath = await this.path(XAI.ChatPath);
      const chatPayload = {
        method: "POST",
