...
import { getClientConfigSync } from "@/app/config/client";
  ...
  path(path: string): string {
    ...
    if (baseUrl.length === 0) {
      try {
        ...
        console.error("[ChatGLM] Failed to get client config:", error);
        baseUrl = ApiPath.ChatGLM; // Default to API path if config fetch fails
      }
    }
