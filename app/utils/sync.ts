...
const MergeStates: StateMerger = {
  ...
  [StoreKey.Mask]: (localState, remoteState) => {
    ...
    // 處理分類順序
    if (remoteState.categoryOrder && remoteState.categoryOrder.length > 0) {
      // 如果遠程有分類順序，則使用遠程的
      localState.categoryOrder = remoteState.categoryOrder;
      
      // 同時更新本地存儲
      try {
        localStorage.setItem("mask-category-order", JSON.stringify(remoteState.categoryOrder));
      } catch (e) {
        console.error("Failed to save remote category order to localStorage", e);
      }
    } else if (localState.categoryOrder && localState.categoryOrder.length > 0) {
      // 如果本地有分類順序但遠程沒有，保留本地的
      try {
        localStorage.setItem("mask-category-order", JSON.stringify(localState.categoryOrder));
      } catch (e) {
        console.error("Failed to save local category order to localStorage", e);
      }
    }
