...
function LanguageSelect() {
  ...
  return (
    <Select
      value={getLang()}
      onChange={async (e) => {
        const newLang = e.currentTarget.value as Lang;
        
        try {
          // 1. 更新配置
          await config.update((config: any) => {
            config.lang = newLang;
          });

          // 2. 保存到 localStorage (不重新載入)
          setItem(LANG_KEY, newLang);

          // 3. 同步到 upstash (語言切換需要立即同步)
          await syncStore.sync();

          // 4. 最後才重新載入頁面
          location.reload();
        } catch (e) {
          console.error("[Lang] failed to update language:", e);
          showToast(Locale.Settings.Sync.Fail);
        }
      }}
    >
...
function ModelProviderStatus() {
  ...
  const getProviderStatus = (provider: string, key: string, url: string) => {
    ...
    return {
      name: provider,
      ready: hasKey && hasUrl,
      status: hasKey && hasUrl ? "運行中" : (hasKey ? "缺少端點" : (hasUrl ? "缺少 API 金鑰" : "未設置")),
      icon: hasKey && hasUrl ? "🟢" : (hasKey || hasUrl ? "🟠" : "🔴")
    };
  };
  
  const providers = [
    getProviderStatus("OpenAI", accessStore.openaiApiKey || accessStore.defaultOpenaiApiKey, accessStore.openaiUrl),
    // getProviderStatus("Azure", accessStore.azureApiKey, accessStore.azureUrl),
    getProviderStatus("Google", accessStore.googleApiKey, accessStore.googleUrl),
    getProviderStatus("Anthropic", accessStore.anthropicApiKey, accessStore.anthropicUrl)
  ];
  
  return (
    <List>
      <ListItem
        title={Locale.Settings.Provider.Status}
        subTitle={Locale.Settings.Provider.SubStatus}
      >
        <div className={styles["provider-status"]}>
          {providers.map((provider, index) => (
            <div key={index} className={styles["provider-item"]}>
              <div className={styles["provider-icon"]}>{provider.icon}</div>
              <div className={styles["provider-info"]}>
                <div className={styles["provider-name"]}>{provider.name}</div>
                <div className={styles["provider-status-text"]}>{provider.status}</div>
              </div>
            </div>
          ))}
        </div>
...
export function Settings() {
  ...
  // 檢查是否有數據庫 API key
  const hasDbApiKey = !!accessStore.defaultOpenaiApiKey && accessStore.defaultOpenaiApiKey.trim().length > 0;
    ...
    () =>
      debounce(async () => {
        try {
          ...
          console.error("[Settings] failed to sync:", e);
          showToast(Locale.Settings.Sync.Fail);
        }
      }, 1000),
    [syncStore]
  ...
  const updateConfig = useCallback((updater: (config: any) => void) => {
    config.update(updater);
    debouncedSync();
  }, [config, debouncedSync]);
  ...
  const currentVersion = updateStore.version ? updateStore.formatVersion(updateStore.version) : "";
  const remoteId = updateStore.remoteVersion ? updateStore.formatVersion(updateStore.remoteVersion) : "";
  const hasNewVersion = currentVersion && remoteId ? semverCompare(currentVersion, remoteId) === -1 : false;
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        console.error("[Settings] failed to fetch client config:", error);
      }
    };
    fetchConfig();
  }, []);
  ...
  // const XAIConfigComponent = accessStore.provider ===  && (
  //   <>
  //     <ListItem
